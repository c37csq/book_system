...
class HomeController extends Controller {
  ...
  async addBook () {
    ...
    let bookImg = 'http://127.0.0.1:7004/public/uploads/' + img
  ...
  async updateBook () {
    ...
    let bookDesc = this.ctx.request.body.detail
    let bookImg = 'http://127.0.0.1:7004/public/uploads/' + imgName
    const sql = " SELECT id FROM type WHERE typeName = '" + typeName + "'"
    const res = await this.app.mysql.query(sql)
    let type = res[0].id
  ...
  async githubLogin () {
    ...
    var clientID = 'b1ef73e0b7198ca0b6b4'
    var clientSecret = '2a6f923018e9974691bd49e27f1e96b5a736804c'
    ...
    const selectResult = await this.app.mysql.query(sql)
  ...
  async baiduLogin () {
    var code = this.ctx.query.code
    var api_key = 'MCUADfCpV4s3PUNEkxFKc876'
    var secret_key = 'FsZjWTNDpDbnVGcGxGf3F32gnOX5Yg3c'
    var redirect_uri = 'http://localhost:3000/#/home'
    var url = encodeURI(`https://openapi.baidu.com/oauth/2.0/token?grant_type=authorization_code&code=${code}&client_id=${api_key}&client_secret=${secret_key}&redirect_uri=${redirect_uri}`)
    var res = await this.ctx.curl(url, {
      method: 'POST',
      headers: {
        accept: 'application/json'
    ...
    let accessToken = res.data.access_token
    var result = await this.ctx.curl(encodeURI('https://openapi.baidu.com/rest/2.0/passport/users/getInfo?access_token=' + accessToken), {
      method: 'GET',
      headers: {
        accept: 'application/json'
      },
    ...
    let id = result.data.portrait.replace(/[^1-9]/ig,"")
    // let id = result.data.openid
